...
class HealthCheckComponent(BaseModel):
    ...
    """ # noqa: E501
    id: Annotated[str, Field(strict=True, max_length=50)] = Field(description="The id of the component.")
    name: Annotated[str, Field(strict=True, max_length=150)] = Field(description="The name of the component.")
    type: StrictStr = Field(description="The type of component affected when `status` is `WARN` or `DOWN`. The possible values are: - `ADAPTER`: The reported status is for the Service Adapter. - `API`: The reported status is for the Service Provider. - `DATABASE`: The reported status is for a database dependency of the Service Provider. - `SYSTEM`: The reported status is for any other known system component such as cache or gateway. - `OTHER`: The reported status is either for a component that does not belong to the types described above or the source of the issue is unknown. ")
    description: Optional[Annotated[str, Field(strict=True, max_length=250)]] = Field(default=None, description="The detailed status of the component.")
    status: HealthStatus
    endpoints: Optional[List[StrictStr]] = Field(default=None, description="When the `status` of the component is not `UP`, then the list of endpoints affected.")
    __properties: ClassVar[List[str]] = ["id", "name", "type", "description", "status", "endpoints"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['OTHER', 'SYSTEM', 'API', 'ADAPTER', 'DATABASE']):
            raise ValueError("must be one of enum values ('OTHER', 'SYSTEM', 'API', 'ADAPTER', 'DATABASE')")
        return value
