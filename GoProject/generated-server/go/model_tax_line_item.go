...
// AssertTaxLineItemRequired checks if the required fields are not zero-ed
func AssertTaxLineItemRequired(obj TaxLineItem) error {
	...
	if err := AssertTaxJurisdictionRequired(obj.Jurisdiction); err != nil {
		return err
	}
	return nil
}

// AssertTaxLineItemConstraints checks if the values respects the defined constraints
func AssertTaxLineItemConstraints(obj TaxLineItem) error {
	if obj.Number < 1 {
		return &ParsingError{Param: "Number", Err: errors.New(errMsgMinValueConstraint)}
	}
	if err := AssertTaxJurisdictionConstraints(obj.Jurisdiction); err != nil {
		return err
	}
	if obj.Rate > 100 {
		return &ParsingError{Param: "Rate", Err: errors.New(errMsgMaxValueConstraint)}
	}
	return nil
}
