...
// ValidationErrorResponse - The error response for validation errors with the respective entity and its field information.
type ValidationErrorResponse struct {

	Errors []ValidationErrorResponseErrorsInner `json:"errors"`
}

// AssertValidationErrorResponseRequired checks if the required fields are not zero-ed
func AssertValidationErrorResponseRequired(obj ValidationErrorResponse) error {
	...
	for _, el := range obj.Errors {
		if err := AssertValidationErrorResponseErrorsInnerRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertValidationErrorResponseConstraints checks if the values respects the defined constraints
func AssertValidationErrorResponseConstraints(obj ValidationErrorResponse) error {
	for _, el := range obj.Errors {
		if err := AssertValidationErrorResponseErrorsInnerConstraints(el); err != nil {
			return err
		}
	}
	return nil
